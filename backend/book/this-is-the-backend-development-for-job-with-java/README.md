...
### 12. 상품 관리 애플리케이션에 객체지향 더하기
1) 클래스 추상화하기
   - interface: 추상 메서드와 상수만을 가질 수 있는 클래스, 다중 상속을 지원, implements 키워드로 구현
   - `@Profile`: 프로파일을 지정하여 빈을 등록, `@Profile("test")`로 지정하면 test 프로파일일 때만 빈을 등록
   - `-Dspring.profiles.active=test`: 프로파일을 지정하여 애플리케이션 실행, `application-test.properties` 파일을 찾아 프로퍼티를 로드, `applicationt.properties`에 설정된 Profile보다 우선순위가 높다.
   - 의존성 주입(DI): 객체 간의 의존 관계를 외부에서 주입하는 것
   - 의존성 역전 원칙(DIP): 고수준 모듈은 저수준 모듈에 의존하면 안된다. 둘 다 추상화에 의존해야 한다. 추상화는 세부사항에 의존하면 안된다. 세부사항은 추상화에 의존해야 한다.


2) 리팩토링과 테스트 코드
   - 리팩토링: 코드의 구조를 변경하지 않고 내부 구조를 개선하는 것
   - 테스트 코드: 코드의 품질을 높이기 위해 작성하는 코드, 코드의 유지보수가 쉬워진다. 작성한 테스트 코드에 한해 기존과 동일하게 작동한다는 사실만 보장한다. 테스트 코드로 작성하지 않은 부분에서 버그가 발생할 수 있다.
   - 동일성: 두 객체가 같은 객체인지 비교하는 것, 동등성: 두 객체가 같은 값을 가지고 있는지 비교하는 것
     - 비교시 일정 범위의 Long 등 인스턴스는 캐시를 생성하여 재사용한다. -128부터 127 범위의 정수 값을 캐시.
   - 얕은 복사: 객체의 주소값만 복사, 깊은 복사: 실제 값을 새로운 메모리 공간에 복사


3) 모킹과 단위 테스트
   - 통합 테스트: 여러 컴포넌트를 하나로 묶어 테스트, 단위 테스트: 하나의 컴포넌트를 테스트
   - 모킹: 테스트 대상이 의존하는 객체를 가짜 객체로 대체하는 것
   - Mockito: 자바에서 가장 많이 사용되는 모킹 프레임워크, `@Mock`, `@InjectMocks`, verify(): 메서드 호출 여부 검증
...
2) 문제 풀어 보기
   - URL 경로의 네이밍
     - 카멜 케이스(Camel Case): 첫 단어를 제외한 나머지 단어의 첫 글자를 대문자로 표기, 대소문자를 구분하지 않는 서버에서 정상 작동하지 않을 수 있고, 알파벳이 아닌 문자를 표현할 때는 띄어쓰기를 할 수 없다는 단점
     - 척추 케이스(Spinal Case): 모든 단어를 소문자로 표기하고 단어 사이에 하이픈(-)를 사용, 일반적으로 가장 권장하는 방법
     - 스네이크 케이스(Snake Case): 모든 단어를 소문자로 표기하고 단어 사이에 언더바(_)를 사용, 일부 검색엔진에서는 언더바를 띄어쓰기가 아니라 하나의 단어로 인식하는 경우 발생
   - `@WebMvcTest`: 스프링 부트에서 제공하는 테스트용 애노테이션, 웹 애플리케이션 MVC 컨트롤러 테스트를 진행, `@WebMvcTest(ShortUrlController.class)`로 특정 컨트롤러만 테스트
     - `@MockBean`: 특정 빈을 목 객체로 대체
     - `MockMcv`: 스프링 MVC 테스트를 위한 클래스, HTTP 요청을 작성하고 응답을 검증
